...
SCAN_UNROLL="${TRAIN_SCAN_UNROLL:-4}"
MEMORY_BACKEND="${TRAIN_MEMORY_BACKEND:-latent_recurrent}"
SPACETIME_DEPTH="${TRAIN_SPACETIME_DEPTH:-4}"
TEMPORAL_KERNEL_SIZE="${TRAIN_TEMPORAL_KERNEL_SIZE:-5}"
FNO_DEPTH="${TRAIN_FNO_DEPTH:-4}"
...
SUPERVISED_ONLY="${TRAIN_SUPERVISED_ONLY:-0}"
STEPS_PER_EPOCH="${TRAIN_STEPS_PER_EPOCH:-30}"
TRAIN_CASE_LIMITS="${TRAIN_CASE_LIMITS:-}"
...
INPUT_SCALING="${TRAIN_INPUT_SCALING:-fixed_training_scale}"
DYNAMIC_AMPLITUDE_FLOOR="${TRAIN_DYNAMIC_AMPLITUDE_FLOOR:-1e-6}"
ALLOW_UNIFORM_HEATING="${TRAIN_ALLOW_UNIFORM_HEATING:-0}"
...
ARGS=(
  ...
  --scan-unroll "${SCAN_UNROLL}"
  --memory-backend "${MEMORY_BACKEND}"
  --spacetime-depth "${SPACETIME_DEPTH}"
  ...
  --train-case-limits "${TRAIN_CASE_LIMITS}"
  ...
  --dynamic-amplitude-floor "${DYNAMIC_AMPLITUDE_FLOOR}"
...
if [[ "${ALLOW_UNIFORM_HEATING}" == "1" ]]; then
  ARGS+=(--allow-uniform-heating)
fi
if [[ "${SUPERVISED_ONLY}" == "1" ]]; then
  ARGS+=(--supervised-only)
fi
