...
DYNAMIC_AMPLITUDE_SCALING="${TRAIN_DYNAMIC_AMPLITUDE_SCALING:-0}"
CLOSURE_HISTORY_INPUT="${TRAIN_CLOSURE_HISTORY_INPUT:-0}"
RELATIVE_TRAJECTORY_LOSS="${TRAIN_RELATIVE_TRAJECTORY_LOSS:-0}"
HEAT_FLUX_BOUND="${TRAIN_NORMALIZED_HEAT_FLUX_BOUND:-8.0}"
...
if [[ "${DYNAMIC_AMPLITUDE_SCALING}" == "1" ]]; then
  ARGS+=(--dynamic-amplitude-scaling)
fi
