...
INTERFACE_FLUX_ROLLOUT_TAIL_CHAIN_LOSS_BACKEND = "exact_fourier_hermite_q_rollout_tail_chain"
INTERFACE_FLUX_ROLLOUT_CHAIN_ONLY_LOSS_BACKEND = "exact_fourier_hermite_q_rollout_chain_only"
INTERFACE_FLUX_ROLLOUT_RECURSIVE_LIFT_LOSS_BACKEND = "exact_fourier_hermite_q_rollout_recursive_lift"
INTERFACE_FLUX_ROLLOUT_HISTORY_LIFT_LOSS_BACKEND = "exact_fourier_hermite_q_rollout_history_lift"
INTERFACE_FLUX_ROLLOUT_HISTORY_LIFT_XV_RES_LOSS_BACKEND = "exact_fourier_hermite_q_rollout_history_lift_xv_res"
...
def build_exact_f_rollout_cache_metadata(
    ...
    **kwargs,
    ...
    metadata = build_interface_flux_rollout_cache_metadata(**kwargs)
...
def _run_exact_f_rollout_reference_history(
    ...
    *,
    ...
    return {
        interface_flux_rollout_coeff_key(projection_order): (coeff_real + 1j * coeff_imag).astype(np.complex64),
...
def _interface_flux_rollout_split_lengths(history_length: int, val_fraction: float) -> Tuple[int, int]:
    ...
    *,
    ...
    if (
        ...
        if int(ref_q_windows.ndim) != 3:
            raise ValueError("tail-chain interface-flux rollout requires all-k q windows")
    ...
    def make_loss_fn_for_target(target_nv: int):
        ...
        def loss_fn_for_target(
            ...
            learned = build_learned_interface_closure(
                ...
                n_low=n_low, training_mode=INTERFACE_FLUX_ROLLOUT_TRAINING_MODE,
    ...
    def make_loss_fn_for_target(target_nv: int, *, chain_only: bool = False):
        ...
        def loss_fn_for_target(
            ...
            learned = build_learned_interface_closure(
                ...
                loss_backend=(
                    INTERFACE_FLUX_ROLLOUT_CHAIN_ONLY_LOSS_BACKEND
                    if bool(tail_chain_only)
                    else INTERFACE_FLUX_ROLLOUT_TAIL_CHAIN_LOSS_BACKEND
                    if bool(tail_chain_enabled)
                    else INTERFACE_FLUX_ROLLOUT_LOSS_BACKEND
                ),
                lambda_q=1.0,
                lambda_E=0.0,
                lambda_dist=0.0,
            ...
            for weight, regime in zip(weight_arr, active_regimes):
                batch = regime_batches[regime]
                if regime == REGIME_LINEAR:
                    _, q_loss, chain_loss = interface_flux_rollout_loss_for_anchor_batch(
...
def make_interface_flux_rollout_recursive_lift_batch_loss(
    *,
    ...
    def make_loss_fn_for_target(target_nv: int):
        ...
        def loss_fn_for_target(
            ...
            lift_learned = build_learned_interface_closure(
                ...
                context_mode=context_mode,
                rollout_horizon=rollout_horizon,
                loss_backend=INTERFACE_FLUX_ROLLOUT_RECURSIVE_LIFT_LOSS_BACKEND,
...
def make_interface_flux_rollout_history_lift_batch_loss(
    *,
    base_params: Dict[str, Array],
    regime_weights: Dict[str, float],
    Nm: int,
    ...
    def make_loss_fn_for_target(target_nv: int):
        ...
        def loss_fn_for_target(
            ...
            history_learned = build_learned_interface_closure(
                ...
                context_mode=context_mode,
                rollout_horizon=rollout_horizon,
                loss_backend=INTERFACE_FLUX_ROLLOUT_HISTORY_LIFT_LOSS_BACKEND,
...
def online_training_log_components(
    *,
    ...
    if str(train_objective) == EXACT_F_ROLLOUT_OBJECTIVE:
        return ("f",)
    if str(train_objective) == INTERFACE_FLUX_ROLLOUT_OBJECTIVE:
        if str(online_loss_backend) in {
            INTERFACE_FLUX_ROLLOUT_HISTORY_LIFT_LOSS_BACKEND,
            INTERFACE_FLUX_ROLLOUT_HISTORY_LIFT_XV_RES_LOSS_BACKEND,
        }:
            return ("hist",)
        if str(online_loss_backend) == INTERFACE_FLUX_ROLLOUT_RECURSIVE_LIFT_LOSS_BACKEND:
            return ("chain",)
        if str(online_loss_backend) == INTERFACE_FLUX_ROLLOUT_CHAIN_ONLY_LOSS_BACKEND:
            return ("chain",)
        if str(online_loss_backend) == INTERFACE_FLUX_ROLLOUT_TAIL_CHAIN_LOSS_BACKEND:
            return ("q", "chain")
        return ("q",)
    return ()
        ...
        for step_idx in range(int(steps_per_epoch)):
            if target_nvs:
                if str(target_sampling) == EXACT_TARGET_SAMPLING_CYCLE:
                    target_nv = interface_flux_cutoff_for_step(
                        target_nvs,
                        int(epoch) * int(steps_per_epoch) + int(step_idx),
                    )
                else:
                    target_nv = int(target_nvs[int(rng.integers(0, len(target_nvs)))])
            else:
...
def _legacy_build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        choices=(OFFLINE_TRAINING_MODE, ONLINE_TRAINING_MODE, INTERFACE_FLUX_ROLLOUT_TRAINING_MODE),
    )
    parser.add_argument(
        "--train-objective",
        type=str,
        default="q_only",
        choices=("q_only", INTERFACE_FLUX_ROLLOUT_OBJECTIVE, EXACT_F_ROLLOUT_OBJECTIVE, "trajectory", "trajectory_q_hybrid"),
    )
...
def _legacy_main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    if training_mode in {OFFLINE_TRAINING_MODE, INTERFACE_FLUX_ROLLOUT_TRAINING_MODE} and bool(args.build_dataset_only) and args.dataset_cache is None:
    ...
    exact_active_loss_backend = INTERFACE_FLUX_ROLLOUT_LOSS_BACKEND
    if training_mode == INTERFACE_FLUX_ROLLOUT_TRAINING_MODE:
        ...
        if args.online_reference_cache is not None:
            raise ValueError("interface_flux_rollout does not use --online-reference-cache")
        if bool(args.allow_dataset_cache_nv_superset):
            raise ValueError("interface_flux_rollout does not use --allow-dataset-cache-nv-superset")
        if bool(args.per_target_projection_orders):
            raise ValueError("interface_flux_rollout stores a single max-order superset history; do not pass --per-target-projection-orders")
        if teacher_backend != GRID_CUBIC_SPLINE_TEACHER_BACKEND:
            raise ValueError("interface_flux_rollout only supports teacher_backend=grid_cubic_spline")
        if args.teacher_proj_Nv is not None:
            raise ValueError("interface_flux_rollout computes max(Nv-targets)+1 internally; do not pass --teacher-proj-Nv")
        if args.train_objective not in {INTERFACE_FLUX_ROLLOUT_OBJECTIVE, EXACT_F_ROLLOUT_OBJECTIVE}:
            raise ValueError("interface_flux_rollout requires --train-objective q_rollout or f_rollout")
        if args.train_objective == EXACT_F_ROLLOUT_OBJECTIVE:
            ...
            if (
                ...
                raise ValueError(
                    "symmetry and regime-balanced loss options currently support interface-flux rollout only"
                )
        ...
        if int(args.rollout_horizon) <= 0:
            raise ValueError("interface_flux_rollout requires --rollout-horizon > 0")
        if int(args.rollout_anchor_samples) != 0:
            raise ValueError("interface_flux_rollout does not use --rollout-anchor-samples")
        if int(args.rollout_anchor_pool_size) != 0:
            raise ValueError("interface_flux_rollout does not use --rollout-anchor-pool-size; it samples from the full dense anchor set")
        if int(args.batch_size) <= 0 and not bool(args.build_dataset_only):
            raise ValueError("interface_flux_rollout requires --batch-size > 0")
        ...
        if (
            ...
            raise ValueError(
                "symmetry and regime-balanced loss options currently require interface_flux_rollout"
            )
    ...
    if training_mode == OFFLINE_TRAINING_MODE:
    ...
    elif training_mode == INTERFACE_FLUX_ROLLOUT_TRAINING_MODE:
        ...
        if exact_f_rollout:
            ...
            exact_dataset, max_projection_order = build_interface_flux_rollout_reference_dataset(
                dataset_cache=args.dataset_cache,
                regimes=regimes,
                teacher_Nx=args.teacher_Nx,
                teacher_Nv=args.teacher_Nv,
        ...
        if args.init_checkpoint is not None:
            ...
            if args.k_scale is not None and not math.isclose(float(args.k_scale), float(init_k_scale), rel_tol=1e-12, abs_tol=1e-12):
                raise ValueError("--k-scale must match --init-checkpoint k_scale when warm-starting interface-flux rollout")
            if args.nv_scale is not None and not math.isclose(float(args.nv_scale), float(init_nv_scale), rel_tol=1e-12, abs_tol=1e-12):
                raise ValueError("--nv-scale must match --init-checkpoint nv_scale when warm-starting interface-flux rollout")
            print(f"[train] initialized interface-flux rollout parameters from {args.init_checkpoint}")
        k_scale = (
            float(args.k_scale)
        ...
        if init_stats is not None:
            ...
            if precomputed_stats is None:
                raise RuntimeError("interface-flux rollout expected precomputed stats for index-only training")
        ...
        if exact_q_regime_balanced_loss:
            exact_q_regime_loss_stds = interface_flux_rollout_regime_loss_stds(
                exact_dataset,
                dataset_base,
                max_projection_order=max_projection_order,
            ...
            print(f"[data] interface-flux rollout regime-balanced loss scales: {scale_note}")
        if equilibrium_centered or complex_normalization_mode == "phase_isotropic" or translation_augmented:
            print(
                "[data] interface-flux rollout symmetry constraints: "
        ...
        if tail_chain_enabled:
            ...
            if bool(args.tail_chain_recursive_lift):
                ...
                print(
                    "[data] interface-flux rollout recursive tail lift: "
                ...
                print(
                    "[data] interface-flux rollout virtual tail chain: "
                    f"n_range={display_chain_min}:{tail_chain_n_max} "
                    f"n_batch=all chunk_size={int(args.tail_chain_chunk_size)}"
                )
            if tail_chain_only:
                print("[train] interface-flux rollout stage: chain-only continuation from init checkpoint")
        if tail_history_lift_enabled:
            hist_loss_note = (
            ...
            print(
                "[data] interface-flux rollout history tail lift: "
        ...
        if tail_history_lift_enabled:
            ...
            batch_loss_fn, active_regimes = make_interface_flux_rollout_history_lift_batch_loss(
            ...
            batch_loss_fn, active_regimes = make_interface_flux_rollout_recursive_lift_batch_loss(
                base_params=init_params,
                regime_weights=regime_weights,
                Nm=args.Nm,
                k_scale=k_scale,
            ...
            batch_loss_fn, active_regimes = make_interface_flux_rollout_batch_loss(
                regime_weights=regime_weights,
                Nm=args.Nm,
                k_scale=k_scale,
                nv_scale=nv_scale,
        ...
        if exact_f_rollout:
            ...
            exact_active_loss_backend = (
                INTERFACE_FLUX_ROLLOUT_HISTORY_LIFT_XV_RES_LOSS_BACKEND
                if tail_history_loss_mode == TAIL_HISTORY_LOSS_XV_RES
                else INTERFACE_FLUX_ROLLOUT_HISTORY_LIFT_LOSS_BACKEND
            )
        elif bool(args.tail_chain_recursive_lift):
            exact_active_loss_backend = INTERFACE_FLUX_ROLLOUT_RECURSIVE_LIFT_LOSS_BACKEND
        elif tail_chain_only:
            exact_active_loss_backend = INTERFACE_FLUX_ROLLOUT_CHAIN_ONLY_LOSS_BACKEND
        elif tail_chain_enabled:
            exact_active_loss_backend = INTERFACE_FLUX_ROLLOUT_TAIL_CHAIN_LOSS_BACKEND
        else:
            exact_active_loss_backend = INTERFACE_FLUX_ROLLOUT_LOSS_BACKEND
        steps_per_epoch = int(args.steps_per_epoch)
        if steps_per_epoch <= 0:
        ...
        params, online_component_history = train_with_interface_flux_rollout_minibatch_loss(
            params,
            exact_dataset,
            dataset_base,
            batch_loss_fn,
    ...
    used_lambda_q = args.lambda_q if args.train_objective in {"q_only", INTERFACE_FLUX_ROLLOUT_OBJECTIVE, "trajectory_q_hybrid"} else 0.0
    ...
    metrics_payload: Dict[str, np.ndarray] = {
        ...
        "rollout_horizon": np.array(
            [
                ...
                if (
                    ...
                    or training_mode == INTERFACE_FLUX_ROLLOUT_TRAINING_MODE
                )
                else 0
            ],
            dtype=np.int32,
        ...
        "exact_rollout_precision": np.array(
            [args.exact_rollout_precision if training_mode == INTERFACE_FLUX_ROLLOUT_TRAINING_MODE else ""],
            dtype=np.str_,
        ),
        "exact_target_sampling": np.array(
            [args.exact_target_sampling if training_mode == INTERFACE_FLUX_ROLLOUT_TRAINING_MODE else ""],
            dtype=np.str_,
        ),
    ...
    if training_mode in {ONLINE_TRAINING_MODE, INTERFACE_FLUX_ROLLOUT_TRAINING_MODE}:
    ...
    if training_mode == INTERFACE_FLUX_ROLLOUT_TRAINING_MODE and args.train_objective == EXACT_F_ROLLOUT_OBJECTIVE:
    ...
    elif training_mode == INTERFACE_FLUX_ROLLOUT_TRAINING_MODE and tail_history_lift_enabled:
        if tail_history_loss_mode == TAIL_HISTORY_LOSS_XV_RES:
            exact_loss_annotation = (
    ...
    elif training_mode == INTERFACE_FLUX_ROLLOUT_TRAINING_MODE and tail_chain_enabled:
        display_chain_min = (
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    params, component_history = train_with_interface_flux_rollout_minibatch_loss(
        ...
        seed=args.seed,
        log_components=("q",),
        target_sampling=EXACT_TARGET_SAMPLING_CYCLE,
        profile_trace_dir=args.profile_trace_dir,
        profile_train_steps=args.profile_train_steps,
        profile_skip_steps=args.profile_skip_steps,
    )
