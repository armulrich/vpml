...
# Full-batch q-loss embeds the whole dataset into the JAX/XLA train-step
# executable. T=60, Nv=64+ produces multi-GB constants and can be killed during
# lowering even on large-memory machines. Set TRAIN_BATCH_SIZE=0 explicitly only
# for small diagnostic runs where exact full-batch gradients are intentional.
TRAIN_BATCH_SIZE="${TRAIN_BATCH_SIZE:-8192}"
TRAIN_STEPS_PER_EPOCH="${TRAIN_STEPS_PER_EPOCH:-50}"
