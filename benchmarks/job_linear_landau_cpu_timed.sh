#!/bin/bash
#SBATCH --job-name=landau_cpu
#SBATCH --output=linear_landau_cpu_%A.out
#SBATCH --error=linear_landau_cpu_%A.err
...
# ============================================================================
# Run linear_landau benchmark on CPU and print wall-clock time.
# ============================================================================
#
# Usage (from vpml repo root):
#   sbatch benchmarks/job_linear_landau_cpu_timed.sh
#   sbatch benchmarks/job_linear_landau_cpu_timed.sh /path/to/outdir
# Optional: METHOD=hyper sbatch ...  (default: truncation)
# ============================================================================

CONDA_ENV="${CONDA_ENV:-vpml}"
METHOD="${METHOD:-truncation}"
...
echo "=============================================="
echo "Linear Landau benchmark (CPU, timed), method=$METHOD"
echo "Repo: $VPML_ROOT"
echo "Python: $PYTHON"
echo "Output: $OUT_DIR"
echo "=============================================="

time bash "$VPML_ROOT/benchmarks/run_fig_cpu_timed.sh" linear_landau "$OUT_DIR" --method "$METHOD"
EXIT_CODE=$?
echo ""
