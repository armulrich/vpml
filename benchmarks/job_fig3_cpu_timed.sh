#!/bin/bash
#SBATCH --job-name=fig3_cpu
#SBATCH --output=fig3_cpu_%A.out
#SBATCH --error=fig3_cpu_%A.err
...
# ============================================================================
# Run Fig. 3 benchmark (response-function) on CPU and print wall-clock time.
# ============================================================================
#
# Usage (from vpml repo root):
#   sbatch benchmarks/job_fig3_cpu_timed.sh
#   sbatch benchmarks/job_fig3_cpu_timed.sh /path/to/outdir
#
# Requires a conda env with vpml deps (numpy, jax, matplotlib, scipy).
# Create with: conda create -n vpml python=3.11 && conda activate vpml && pip install numpy jax matplotlib scipy
# ============================================================================
...
echo "=============================================="
echo "Fig. 3 benchmark (CPU, timed)"
echo "Repo: $VPML_ROOT"
echo "Python: $PYTHON"
echo "Output: $OUT_DIR"
echo "=============================================="

time bash "$VPML_ROOT/benchmarks/run_fig_cpu_timed.sh" fig3 "$OUT_DIR"
EXIT_CODE=$?
